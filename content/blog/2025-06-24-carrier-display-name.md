...
### SIM 规则（sim rule / Display Condition）
...
BrandOverride 是最高优先级的 SPN 来源。当 BrandOverride 存在时，所有 PLMN 都被视为 HOME PLMN，且**只显示 SPN**（不显示 PLMN）。这是运营商定制 ROM 时覆盖默认运营商名称的常用手段。
...
## 显示规则计算（getCarrierNameDisplayBitmask）
...
## 场景分析（getCarrierDisplayNameLegacy 分支）
